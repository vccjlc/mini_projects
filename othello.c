/**
 * @file        othello.c
 * @brief       Program to display legal Othello moves.
 *
 * This program enables two players to engage in a game of Othello, focusing
 * solely on legal move display without score tallying or winner determination.
 *
 * Operating in a loop, the program:
 * - Writes a prompt showing the current player's turn and possible legal moves.
 * - Reads the current player's command.
 * - Ends its operation if it reads a command to interrupt the game.
 * - Repeats the loop if it reads a command for the player to pass their turn.
 * - Executes a move if it reads a valid move command, then repeats the loop.
 *
 * The program continues until an interrupt command is given. It does not
 * terminate upon detecting no legal moves. It also doesn't flag errors for
 * passed turns or interrupts when a legal move is possible.
 *
 * The program presumes correct input as per task requirements.
 *
 * Given its basic functionality, detailed comments within the code are omitted.
 */
